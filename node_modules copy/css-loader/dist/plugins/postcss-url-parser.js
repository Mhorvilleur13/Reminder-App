...
function parseDeclaration(declaration, key, result, options) {
  ...
  parsed.walk((valueNode, index, valueNodes) => {
    ...
    if (isUrlFunc.test(valueNode.value)) {
      ...
      const { requestable, needResolve } = shouldHandleURL(url, declaration, result, options); // Do not traverse inside `url`
        ...
        async OnceExit() {
          ...
          const resolvedDeclarations = await Promise.all(
            parsedDeclarations.map(async (parsedDeclaration) => {
              ...
              const resolvedURL = await (0, _utils.resolveRequests)(resolver, options.context, [
                ...new Set([request, url]),
              ]);
