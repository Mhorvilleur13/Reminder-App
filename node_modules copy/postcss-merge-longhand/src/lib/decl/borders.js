  ...
  while (decls.length) {
    ...
    wsc.forEach((d, i) => {
      ...
      const props = nodes.filter(
        (node) => node.prop && names.includes(node.prop) && node.important === lastNode.important
      );
  ...
  rule.walkDecls(/^border($|-(top|right|bottom|left)$)/i, (decl) => {
    ...
    wsc.forEach((d, i) => {
      ...
      mergeRules(rule, [decl.prop, ...props], (rules) => {
        ...
        if (
          longhands[0].value.toLowerCase() === longhands[1].value.toLowerCase() &&
  ...
  while (decls.length) {
    ...
    const lesser = decls.filter(
      (node) =>
        ...
        (node.prop.toLowerCase().includes(lastNode.prop) || node.prop.toLowerCase().endsWith(lastPart))
    );
